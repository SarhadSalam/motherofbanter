    ...
    <div class="media">
        <div class="media-body">
            ...
            @if($images->likes->count() == 0)
                <li><a href="{{ route('image.like', ['imageId' => $images->id]) }}"><i class="unused-icon icon icon-thumbs-o-up"></i></a></li>
            @else
                 <li><a href="{{ route('image.like', ['imageId' => $images->id]) }}"><i class="icon icon-thumbs-o-up"></i></a></li>
            @endif
            @if(Auth::user()->dislikes->count() == 0)
                <li><a href="{{ route('image.dislike', ['imageId' => $images->id]) }}"><i class="unused-icon icon icon-thumbs-o-down"></i></a></li>
            @else
                <li><a href="{{ route('image.dislike', ['imageId' => $images->id]) }}"><i class="icon icon-thumbs-o-down"></i></a></li>
            @endif
            ...
            <ul class="list-inline big-icon">
                ...
                <li class="pull-right"><a href="{{route('auth.signin')}}">{{ $images->created_at->diffForHumans()}}</a></li>
                <li class="pull-right"><a href="{{route('auth.signin')}}">{{$images->dislikes->count()}}  {{str_plural('dislike', $images->dislikes->count())}}</a></li>
                <li class="pull-right"><a href="{{route('auth.signin')}}">{{$images->likes->count()}}  {{str_plural('like', $images->likes->count())}}</a></li>
            </ul>
            @endif
            <hr>
        </div>
