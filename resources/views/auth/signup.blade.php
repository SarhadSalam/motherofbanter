	...
	<div class="container">
		<form class="form-signin" role="form" method="post" action="{{route ('auth.signup') }}" data-parsley-validate>
			...
			<div class="form-group{{ $errors->has('username') ? ' has-error' : ''}}">
				...
				<input type="text" name="username" class="form-control" placeholder="Please Choose an Username"
					   id="username" value="{{ Request::old('username') ?: '' }}" required=""
			...
			<div class="form-group{{ $errors->has('password') ? ' has-error' : ''}}">
				...
				<input type="password" name="password" class="form-control" placeholder="Please Choose A Password"
					   id="password" value="" required="" data-parsley-required-message="Password is required"
					   data-parsley-trigger="change focusout" data-parsley-minlength="6" data-parsley-maxlength="20">
			...
			<div class="form-group{{ $errors->has('g-recaptcha') ? ' has-error' : ''}}"
				...
				<div class="g-recaptcha" data-sitekey="{{ env('RE_CAPTCHA_SITEKEY') }}"
					 data-parsley-required-message="The recaptcha is required" required=""></div>
		...
		<div class="vertical-buttons">
			<h4>Or</h4>
			<a href="{{ route('social.redirect', ['provider' => 'facebook']) }}" class="btn btn-lg btn-block facebook"
			   display="popup">Login With Facebook</a>
