...
@section('content')
	<div class="container">
		<form class="form-signin" role="form" method="POST"
			  action="{{ route ('reset.password', ['code' => $reset_code->reset_token])}}" data-parsley-validate>
			<h2 class="form-signin-heading">Reset Password:</h2>
			<div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
				<label for="email" class="sr-only">Email</label>
				<input type="email" name="email" class="form-control" id="email"
					   value="{{ Request::old('email') ?: '' }}" placeholder="Email" autofocus required=""
					   data-parsley-required-message="Email is required" data-parsley-trigger="change focusout"
			...
			<div class="form-group{{ $errors->has('password') ? ' has-error' : ''}}">
				...
				<input type="password" name="password" class="form-control" placeholder="Please Choose A New Password"
