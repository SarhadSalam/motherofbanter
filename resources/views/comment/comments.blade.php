...
@if (Auth::check())
	...
	<div class="col-lg-8 comment-box">
		...
		<form role="form"
		      action="{{ route('image.reply', ['imageId' =>$images->id] )}}"
		      method="post"
			...
			<div class="panel panel-default">
				...
				<div class="panel-body{{ $errors->has(" reply-{$images->id}") ? ' has-error': '' }}">
							...
							<textarea id="commentArea"
							          ...
							          name="reply-{{ $images->id }}"
							          class="form-authentication__control comment-emoji-area"
							          rows="3"
							          required=""
							          data-parsley-required-message="You must add a comment!"
...
@include('comment.user_comments'){{-- Comment Box --}}
